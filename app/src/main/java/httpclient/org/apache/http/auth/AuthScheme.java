 ...
 *
 */
package httpclient.org.apache.http.auth;

import httpcore.org.apache.http.Header;
import httpcore.org.apache.http.HttpRequest;

/**
 * HTTP认证采用 质询-响应（challenge-response） 的机制。
 * 质询 是服务器端对客户端的质询，即要求客户端发送认证信息；
 * 响应 是客户端对 质询 的响应，即发送带有认证信息的HTTP响应
 *
 * 该接口代表 面向认证方案的一种抽象质询响应机制
 * <p>
 * 一个认证方案必须支持一下功能：
 * <ul>
 *   <li>客户端请求一个受保护的资源时，该方案要能解析和处理响应中目标服务器端对客户端的质询，
 *   <li>提供文本名称
 *   <li>如果可能，提供参数
 *   <li>如果可能，提供这个认证方案要应用的realm
